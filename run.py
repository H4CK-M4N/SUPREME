...
f"""{Hj}
 ...
 - TIKTOK      : mela_nin23
 - FANSPAGE    : whatsApp.com/100022086172556
 - WHATSAPP    : +2349150557103
 - GITHUB      : github.com/M3L4NIN-H4CK3R
 - SCRIPT NAME : SUPREME
 - VERSION     : 1.3
 - UPDATE DATE : 21 Februari 2022
...
def Masuk():
	try:
		...
		os.system('clear')
		banner()
		print ('\n%s%s%s 01 %sLogin instagram (crack account instagram) '%(U,til,K,O))
		print ('%s%s%s 02 %sLogin via cookie (crack account facebook) '%(U,til,K,O))
		print ('%s%s%s 03 %sMethod get cookie facebook '%(U,til,K,O))
...
class Menu():
	...
	def tentang(self):
		...
		if "mbasic_logout_button" not in a:
			...
			print ('%s•%s 01 %sCrack from list friend '%(U,P,O))
			print ('%s•%s 02 %sCrack from total follower'%(U,P,O))
			...
			print ('%s•%s 07 %sCrack from message mesengger'%(U,P,O))
			print ('%s•%s 08 %sCrack from saran friend'%(U,P,O))
			print ('%s•%s 09 %sCrack user instagram %spro'%(U,P,O,H))
			#print ('%s•%s 10 %sSetting user agent'%(U,P,O))
			print ('%s•%s 10 %sSee results crack'%(U,P,O))
...
class instagram:
	...
	def logo(self):
		...
		banner()
		print(f"""
{U}•{P} 01 {O}Crack from follower
{U}•{P} 02 {O}Crack from follow 
{U}•{P} 03 {O}Crack from search name
{U}•{P} 04 {O}Crack secara target 
{U}•{P} 05 {O}Check status account crack
{U}•{P} 06 {O}Bot auto unfollow
{U}•{P} rm {O}Wipe account
{U}•{M} 00 {O}Logout
	""")
